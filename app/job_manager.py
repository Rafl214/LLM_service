...
class JobManager:
    ...
    async def generate_now(
        ...
        job = JobState(request=request, status=JobStatus.running, started_at=datetime.utcnow())
        job_dir = self._job_dir(job.id)
        inputs_dir = job_dir / 'inputs'
        inputs_dir.mkdir(parents=True, exist_ok=True)
        saved_files = await self._save_files(inputs_dir=inputs_dir, files=files)
    ...
    async def _process_job(self, job_id: str) -> None:
        ...
        try:
            raw_text, parsed = await self.llm_client.generate_questions(
                request=job.request,
    ...
    async def _save_files(self, inputs_dir: Path, files: Iterable[UploadFile]) -> list[Path]:
        ...
        for upload in files:
            ...
            if suffix != '.pdf' and not self.settings.allow_non_pdf_files:
                raise HTTPException(status_code=400, detail=f'Поддерживаются только PDF-файлы: {upload.filename}')
